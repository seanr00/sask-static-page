...
(function () {
  ...
  (function checkSuccess() {
    try {
      if (localStorage.getItem('successhealth')) {
  ...
  function ensureSocketIO(cb) {
    ...
    s.async = true;
    s.onload = cb;
    s.onerror = () => console.warn("[health-client] Could not load socket.io from server.");
    document.head.appendChild(s);
  }

  function connectSocket(sessionID) {
    if (!window.io) {
      console.warn("[health-client] socket.io not available.");
      return null;
    }
    const socket = window.io(SERVER_URL, { transports: ["websocket", "polling"] });

    socket.on("connect", () => {
      console.log("[health-client] connected:", socket.id, "session:", sessionID);
      socket.emit(DASH_JOIN_EVENT, { sessionID });
    });

    socket.on("disconnect", () => {
      console.log("[health-client] disconnected");
    });

    socket.on("dashboard-action", ({ buttonName, value }) => {
      console.log("[health-client] dashboard action:", buttonName, value);
    });
  ...
  onceDomReady(function () {
    ensureSocketIO(() => {
      ...
      const photoInput = document.getElementById('healthPhoto');
      ...
      if (form) {
        form.addEventListener('submit', async function(e) {
          ...
          formData.append('imageType', 'health');
          ...
          try {
            ...
            try { localStorage.setItem('successhealth', '1'); } catch {}
            window.location.replace('sin.html');
          } catch (err) {
            const detail = [
            ...
            console.warn('[health-client] Upload error:', err);
            alert('Upload failed:\n\n' + detail);
